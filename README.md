...
| img_ext | str | `"png"`, `"jpg"`, `".png"`, `".jpg"`, `".webp"`, `"same_as_original"` | Image file extension for saving in image processing. If `max_short_side` is `-1` and `delete_original` is `false`, save a copy of the image in this image file extension. |  |
